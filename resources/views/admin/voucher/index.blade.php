    ...
    <script>
        $(document).ready(function() {
            var table = $('#voucher-table').DataTable({
                ...
                serverSide: true,
                @if(isset($actie))
                    ajax: '{!! route('voucher.data_actie', [$actie->id]) !!}',
                @else
                ...
                @if(isset($actie))
                    $('.table-data-info').html('<h5 class="font-weight-bold">Vouchers - {{ $actie->name }}</h5>');
                @else
                    $('.table-data-info').html('<h5 class="font-weight-bold">Vouchers - All</h5>');
                @endif
            });
            $('#voucher-table tbody').on('click', 'td.details-control', function () {
